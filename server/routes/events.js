...
// Get related study sessions for an event
router.get('/related/:eventId', async (req, res) => {
  try {
    const eventId = req.params.eventId;
    console.log(`Fetching study sessions related to event ID: ${eventId}`);
    ...
    res.json(relatedStudySessions);
  } catch (error) {
    console.error('Error fetching related study sessions:', error);
    res.status(500).json({ message: 'Server error' });
  }
});
